    ...
    function process_html() {
        ...
        if (semver.gte(CONFIG.apiVersion, "1.36.0")) {
            ...
            $.each(selectProfileValues, function(key, value) {
                if (key != CONFIG.profile) selectProfile.append(new Option(value, key));
            });
            $.each(selectRateProfileValues, function(key, value) {
                if (key != CONFIG.rateProfile) selectRateProfile.append(new Option(value, key));
            });
...
TABS.pid_tuning.updateRatesLabels = function() {
    ...
    if (!self.rateCurve.useLegacyCurve && self.rateCurve.maxAngularVel) {
        ...
        if (rcStickElement) {
            ...
            stickContext.save();
            var maxAngularVelRoll = self.maxAngularVelRollElement.text() + ' deg/s',
